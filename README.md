...
```bash
scannerApp$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
scannerApp$ sam local start-api
scannerApp$ curl http://localhost:3000/classify_digit
```
...
```bash
scannerApp$ sam logs -n InferenceFunction --stack-name scannerApp --tail
```
...
```bash
aws cloudformation delete-stack --stack-name scannerApp
```
